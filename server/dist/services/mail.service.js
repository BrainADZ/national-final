...
exports.sendAdsEnquiryMail = sendAdsEnquiryMail;
...
async function sendAdsEnquiryMail(params) {
    const transporter = getTransport();
    const to = process.env.ENQUIRY_ADMIN_EMAIL;
    if (!to)
        throw new Error("ENQUIRY_ADMIN_EMAIL missing in env.");
    ...
    const subject = `New Ads Enquiry - ${params.fullName}${params.company ? ` (${params.company})` : ""}`;
    ...
    <div style="max-width:680px;margin:0 auto;background:#ffffff;border:1px solid #e6e8eb;border-radius:12px;overflow:hidden;">
      ...
      <div style="padding:20px;">
        <table cellpadding="0" cellspacing="0" width="100%" style="border-collapse:collapse;">
          ...
          ${row("Phone", safePhone)}
          ${params.email
        ? row("Email", `<a href="mailto:${safeEmail}" style="color:#0b5fff;text-decoration:none;">${safeEmail}</a>`)
        : ""}
          ...
          ${params.pageUrl
        ? row("Page URL", `<a href="${safePageUrl}" style="color:#0b5fff;text-decoration:none;">${safePageUrl}</a>`)
        : ""}
        </table>
  ...
  `;
    const attachments = [];
    if (params.drawingPath && fs_1.default.existsSync(params.drawingPath)) {
        attachments.push({
            filename: params.drawingName || path_1.default.basename(params.drawingPath),
